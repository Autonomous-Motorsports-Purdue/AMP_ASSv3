...
class ServiceClient(Node):
	def __init__(self):
		...
		while not self.client.wait_for_service(timeout_sec = 5.0):
			self.get_logger().info('service not found and/or available, waiting again...')
...
class KartStateDetermination(Node):
	def __init__(self):
		...
		self.state = "Red" # Change to different deafault string?
		
		self.track_subscription = self.create_subscription(String, "track_state", self.callback, 10)
